...
class Env:
    ...
    def __init__(self, down_lane, width, height, n_veh, n_neighbor):
       ...
       # self.y = [0, 1]  # control variable 1 for v2v and 0 for V2I
        ...
        self.multi_action = self.V2V_h_list + self.V2V_power_dB_List # the power levels # continuous distribution (decisision variables)
        self.sig2_dB = -114
        self.bsAntGain = 8
    ...
    def act(self, states):
        states_tensor = torch.as_tensor(Main_training.state, dtype=torch.float32)
        q_values = self(states_tensor.unsqueeze(0)) # expects a batch dimension , we create a fake batch dimension of size 1 using unsqueeze zero
        max_q_index = torch.argmax(q_values, dim=1)[0]
        action = max_q_index.detach().item() # We extract our new max action from the q_table
        return action
